...
office quarantined. The nightly build's own tests are the publication gate.
...
|---|---|---|
| A red nightly (the 00:02 or 12:02 scheduled build failed) | Yesterday's marts keep serving; the sync publishes yesterday's state. The next day's admission declines until a scheduled build succeeds. | dbt Cloud's own failure alert is the page. If the matcher's rows caused it, remove them (below); otherwise the owner of the failing model. |
...
The nightly's tests gate publication for free: the voter-coverage floor at error severity and the
staging label check at warn. A red nightly leaves yesterday's marts serving, and the next day's
admission declines until a scheduled build succeeds, so a matcher-caused red pauses the loop until the
operator removes the offending rows. There is deliberately no dependency wiring to the sync and no dbt
trigger in the DAG; neither schedule should drift without rechecking this section.

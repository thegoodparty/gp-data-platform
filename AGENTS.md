...
Short, non-obvious context for `gp-data-platform`. The repo overview is in `README.md`; dbt-specific guidance is in `dbt/project/AGENTS.md`.

`AGENTS.md` is the single source of truth. Claude Code reads `CLAUDE.md`, so each directory with an `AGENTS.md` carries a `CLAUDE.md` symlink pointing at it — edit `AGENTS.md`, never `CLAUDE.md`. Run `ai-rules/scripts/agents-md-sync.sh --fix` to create the symlink for a new doc.
...
When reviewing changed code in this repo (e.g. during `/simplify`, `/review`, or any code-review pass), consult the rule files under `ai-rules/` in addition to this file and the per-subproject `AGENTS.md` files.

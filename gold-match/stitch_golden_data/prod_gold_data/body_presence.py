...
# L2 abbreviates generic nouns in its district names (REC, WTR, USD); expanding both sides (the
# office body and the row name go through the same tokens()) means an abbreviation can never leave
# a generic word standing as a mandatory anchor, independent of which types a state happens to load
# -- unlike pooling every dbt type word as generic (tried and reverted: it erased genuine proper-
# name anchors that happen to share a word with some loaded type, e.g. CENTER via Justice_Center).
...
def tokens(text: str) -> list[str]:
    ...
    for t in re.sub(r"[^A-Za-z0-9]+", " ", text or "").upper().split():
        for expanded in WORD_ABBREVIATIONS.get(t, (t,)):
            out.append(str(int(expanded)) if expanded.isdigit() else expanded)
    return out

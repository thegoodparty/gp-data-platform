...
| **Amplitude user_id casts (cross-product)** | Unfiltered joins of `stg_airbyte_source__amplitude_api_events.user_id` (string; holds legacy Firebase-style IDs) to a numeric user_id throw `CAST_INVALID_INPUT` — while time-filtered joins on the same key silently succeed (predicate pushdown removes the malformed rows before the implicit cast). | Cast the product-side key to string (`cast(u.user_id as string) = e.user_id`); never cast the Amplitude side to bigint. | invariant |

...
- **⚠ Two different things are called "activation", and picking the wrong one is the most likely mistake on this page.** `is_activated` / `win_activated_users` is an **outreach** metric: the product performed a voter-outreach send. The end-to-end user view uses a **broader** concept, **Product Output** — the candidate produced something with the product that left it — whose legs are `Candidate Website - Published`, `Voter Outreach - Campaign Completed`, `Voter Data - List Exported`, `Voter Outreach - Phone Banking Call Sheet Downloaded` and `Robocall - Scheduled`. Measured 2026-09-24: Product Output reaches **2,629** users against the OKR's 1,265, and the gap is not outreach. `Candidate Website - Published` alone brings **1,113 users no other leg brings**, so Product Output is majority website publishing. Publishing a website is not voter outreach, which is why the two stayed separate rather than the OKR widening to the larger number. **The two no longer share the `Robocall - Scheduled` leg.** Activation dropped it on 2026-09-24 for the shared send terminal `Voter Outreach - Campaign Scheduled`, because it fires at draft-create on an unpaid row. Product Output still counts it, which is worth re-testing against that concept's own "left the product" rule: an unpaid draft never left.
  ...
  - Two caveats ride on Product Output specifically: three of its five legs went live Jul–Sep 2026, so a pre-2026-07 series carries an instrumentation step rather than growth; and some of its legs are paywalled, which leaks payment into any model using it as a label.

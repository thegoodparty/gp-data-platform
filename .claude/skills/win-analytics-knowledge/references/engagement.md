...
- **Win W+1 Retention (OKR O2 KR1)** = share of Win-ICP users who view the dashboard 8–14 days after their **first voter outreach send** (Amplitude chart `owc6mfnp`: start `ce:Voter Outreach - All`, return `Viewed` path=/dashboard, segment = Win ICP cohort; N-day weekly retention; target 60%). It conditions on activated ICP users **by construction** — weekly cohorts run ~10–25 users — so never read it as a population retention rate. The honest chain: registered → activated (~3–5% all-accounts) → ~55–60% of those return in W+1 (resolved 2026-07-23 from the June All Hands deck + chart definition, after a stakeholder read it against the all-accounts base).

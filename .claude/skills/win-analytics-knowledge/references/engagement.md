...
- **`is_activated`** means **the product performed an outreach send for the user**, which is narrower than it was before 2026-09-24. It is a lifetime flag — anchor it before the election cutoff (`first_campaign_sent_at <= election_date`, or `<= month-end`) for forward/recent cohorts so post-election sends don't leak (mechanism in the point-in-time caveat below). **Activation lands late:** ~59% of ever-activated users send their first campaign >14 days after signup (verified 2026-06-11, 705/1,201), so activation cannot share the 14-day-of-signup window used for the dashboard-view / pledge milestones — anchor it point-in-time to the period end instead. Do not conflate with the colloquial "activated."
- **⚠ Two different things are called "activation", and picking the wrong one is the most likely mistake on this page.** `is_activated` / `win_activated_users` is an **outreach** metric: the product performed a voter-outreach send. The end-to-end user view uses a **broader** concept, **Product Output** — the candidate produced something with the product that left it — now built as `users_win_base.has_product_output` and governed as `win_product_output_users`. Its legs are `Candidate Website - Published`, `Voter Outreach - Campaign Completed` (self-report excluded, as in the OKR), `Voter Data - List Exported`, `Voter Outreach - Phone Banking Call Sheet Downloaded` and the shared send terminal `Voter Outreach - Campaign Scheduled`. Measured against the built model 2026-09-25: Product Output reaches **2,513** users against the OKR's 1,260, and the gap is not outreach. `Candidate Website - Published` alone brings **1,361 users no other leg brings**, so Product Output is majority website publishing. Publishing a website is not voter outreach, which is why the two stayed separate rather than the OKR widening to the larger number.
  - **The two agree on what does not count.** Product Output excludes self-reported outreach by the same `method` qualifier, because a candidate recording what they did elsewhere produced no output here; including it would have read 2,636. It also drops the `Robocall - Scheduled` draft event the OKR dropped on 2026-09-24, for the same reason — it fires at draft-create on an unpaid row, and an unpaid draft never left. That drop cost zero users: all 12 already qualified through another leg.
  - **Which to use:** "did they reach voters" → the OKR. "did they get value out of the product" → Product Output. Never report one as the other, and never compare them in the same series.
  - Two caveats ride on Product Output specifically: three of its five legs went live Jul–Sep 2026, so a pre-2026-07 series carries an instrumentation step rather than growth; and four of the five legs are declared `paywalled`, so any model using it as a label must read `has_free_product_output` instead (**1,666** users, the website leg alone) or it learns who paid and reports it as who engaged.
- **⚠ "Active" needs the machine-emitted filter or it is roughly double.** `int__amplitude_event_catalog.is_machine_emitted` flags events the system emitted on its own behalf: schedulers, broadcasts, status pollers, server-side generation with nobody in the loop, and instrumentation bookkeeping such as experiment assignment. Measured 2026-09-25, the `%Dispatch Skipped` schedulers alone named **1,777 users** as active in the current month who had done nothing, taking it from 3,249 to ~1,500. **Do not substitute `session_id = -1` for this** — over half of `Candidate Website - Published` is server-emitted and it is a real candidate action. The two are told apart by whether a real-session event from the same user lands within five minutes: 0.0–3.5% for the schedulers, 89–91% for the generation events a candidate clicked.
- **Self-reported outreach does not count toward activation**, and this is the single most important thing to know about the metric's history. `Voter Outreach - Campaign Completed` covers three different moments, told apart by its `method` property: absent is the legacy in-product send, `native` is a completed door-knocking walk, and `manual` is a candidate recording outreach they did off-platform. The in-product leg **stopped firing on 2026-09-08** when the flow moved to `outreach/v2/`, and the metric did not visibly fall, because self-report absorbed it — every `Campaign Completed` event after 2026-09-10 is `manual` or `native`. Excluding `manual` moved the mart from 1,338 activated to 1,265 (measured 2026-09-24): 145 users lost activation, every one of them with zero non-self-reported sends, and 72 gained it from legs the old single-event definition never read.
- **Activation is a floor, not a count, until the v2 channels are instrumented.** Three of four outreach channels have no send terminal. Measured over 2026-09-08 to 2026-09-24 against the product DB (`outreach` rows that left `pending_payment` and were not canceled), 121 users made a real send and only 43 fired the anchor event, so event coverage of real senders is **35.5%**, down from 61.3% in the matched prior window. Per channel, users with a committed send vs users never counted: p2p 64/38, phone banking 46/30, social 23/15, door knocking 20/9, robocall 7/4. For a complete send count, read `gp_api_db_outreach` rather than the events.
- **Preparation is not outreach.** Creating a voter list, creating a phone-banking call list, downloading a call sheet and building an audience are all deliberately excluded, as is `Outreach - Phone Banking: Call Logged` — that one is live but fires per call (~26 per caller), and the same predicate feeds `campaigns_sent`, so admitting it would read one caller as 26 campaigns.
...
| Intermediate (Win) | `int__amplitude_win_activity` (monthly grain). A weekly variant (`_weekly`) is in prod `dbt` (coverage `week_start_date` 2025-06-23+). | The recurrent-activity allowlist, which since 2026-09-24 is **derived** from the `anchored_on` declarations of `win_active_candidates_30d` and `win_activated_users` rather than hand-kept in `amplitude_event_is_recurrent`. Six event names today: three generations of dashboard-view event, `Voter Outreach - Campaign Completed`, the shared send terminal `Voter Outreach - Campaign Scheduled`, and `Outreach - Phone Banking: Complete`. (`Outreach - Phone Banking: Call Logged` is **not** in it; it was declined as a leg for its per-call grain, and an earlier revision of this row wrongly listed it and counted seven.) The site-wide `Viewed` page event is intentionally **not** in it (only its `/dashboard` slice counts, which an event-name list cannot express), so consumers must admit that leg separately. **The derivation runs one way only.** The two metrics contribute names to the allowlist; the allowlist does not feed back into them. `win_active_candidates_30d` resolves from `int__amplitude_user_milestones.last_dashboard_viewed_at`, whose intake gates on `is_dashboard_view_event` / `is_outreach_activation_event` directly, so changing a leg of `win_activated_users` cannot move the Active Candidates OKR. One-off lifecycle milestones live in `int__amplitude_user_milestones`. |
...
| Intermediate (journey) | `int__user_product_activity` | One row per gp-api user for the end-to-end user view: first/last activity, lifetime event and active-day counts, dashboard views, onboarding, Product Output (full and free), Serve first/last activity, and the three rolling month flags with `growth_state`. **Admits events by classification, not by name** — everything except `is_machine_emitted` — so it is the one Win activity model that follows the product without an allowlist edit. Activity columns are null, not zero, where `has_amplitude_data` is false; about half the table has no telemetry at all. |

...
| Intermediate (Win) | `int__amplitude_win_activity` (monthly grain). A weekly variant (`_weekly`) is in prod `dbt` (coverage `week_start_date` 2025-06-23+). | The recurrent-activity allowlist, which since 2026-09-24 is **derived** from the `anchored_on` declarations of `win_active_candidates_30d` and `win_activated_users` rather than hand-kept in `amplitude_event_is_recurrent`. Seven event names today: three generations of dashboard-view event, `Voter Outreach - Campaign Completed`, `Robocall - Scheduled`, and both phone-banking `Complete` / `Call Logged`. The site-wide `Viewed` page event is intentionally **not** in it (only its `/dashboard` slice counts, which an event-name list cannot express), so consumers must admit that leg separately. One-off lifecycle milestones live in `int__amplitude_user_milestones`. |

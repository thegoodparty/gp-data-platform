...
| **Win Activated Users** | Count of Win-product users who have activated: the product has performed at least one voter-outreach send for them (first_campaign_sent_at is not null). The activated slice of win_users; the Activated Candidates OKR. Definition owned by users_win_base.is_activated, which is anchored on the outreach-terminal union declared in this metric's config.meta.anchored_on and compiled by is_outreach_activation_event. Two things this deliberately does NOT count. Self-reported outreach, where a candidate records something they did off-platform, is excluded by the method qualifier below: it is a real signal but it is not a send this product made, and counting it let the OKR hold steady through a month in which the in-product leg had stopped firing entirely. And preparation to reach voters (creating a call list, downloading a call sheet, building an audience) is not outreach, so no preparation event is a leg. Three of the four outreach channels have no send terminal yet, so this is a floor rather than a count until they are instrumented. Because the legs went live at different dates, a series over this metric carries instrumentation steps and must not be read as growth. | ref('users_win_base') | [engagement.md](engagement.md) | 2026-08-07 (stale: definition changed since ratification) |

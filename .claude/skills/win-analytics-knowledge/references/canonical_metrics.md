...
| **Corroborated candidate (retrospective)** | Candidacy with a BallotReady or TechSpeed candidacy-stage record AND (general election already happened OR general filing deadline passed); reliable only on completed elections, and "not corroborated" is an upper bound on "not real" (BR under-covers local races) | `candidacy_stage.br_candidacy_id` / `ts_source_candidate_id` rolled to `gp_candidacy_id`, plus `candidacy` election-date / filing deadline | [sources.md](sources.md) | pending |

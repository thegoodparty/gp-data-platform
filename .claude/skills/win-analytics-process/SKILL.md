...
4. **Execute.** Build the working set once with `analytics/lib/win_analysis.py`, then slice every cut from it in pandas (build-once-slice-many; see `methodology.md`). Anchor features point-in-time so post-anchor activity doesn't leak.

...
**Keep framing queries coarse.** Verification and fork-sizing queries stop at existence, coverage windows, and order-of-magnitude counts — enough to resolve a fork, never the analysis's own deliverable. On a simple census-style metric the exact per-bucket table *is* the answer: computing it during framing empties the approval gate, because by the time the human is asked, there is nothing left for their decision to redirect. Leave exact tables to execution.
...
**Canonical metric enumeration.** The dbt **semantic layer is the first-line source of truth for governed status** — when a concept may be a governed metric, check `dbt/project/models/**/sem_*.yml` `config.meta` (definition, `owner`) and `analytics/diagnostics/semantic_catalog/config/ratifications.yml` (two independent sign-off halves, `business` for the rule and `data` for the build; an absent half means that half is pending) **before** the projected `canonical_metrics.md`, which is a generated projection that can lag both (observed 2026-08: a metric ratified in the yml still read `pending` in the projection). Then, before defining a *new* engagement metric, list the team's existing canonical metrics from the product knowledge skill's `canonical_metrics.md`, and explain why a new one is needed instead of using or extending these.

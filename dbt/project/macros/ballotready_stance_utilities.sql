{% macro get_stance_freshness() %}
  ...
  with candidacy_updates as (
    select
      candidacy_id,
      max(updated_at) as last_candidacy_update
  ...
  stance_updates as (
    select
      candidacy_id,
      max(updated_at) as last_stance_update
  ...
  select
    ...
    {{ dbt.datediff('s.last_stance_update', 'c.last_candidacy_update', 'day') }} as days_difference
  from candidacy_updates c
  left join stance_updates s on c.candidacy_id = s.candidacy_id
  where {{ dbt.datediff('s.last_stance_update', 'c.last_candidacy_update', 'day') }} > 14
...
{% macro monitor_stance_counts() %}
  ...
  with stance_count as (
    select
      candidacy_id,
      array_size(stances) as num_stances,
      updated_at
    from {{ ref('int__ballotready_stance') }}
  ),
  ...
  where
    -- Detect outliers: stances that are less than (Q1 - 1.5*IQR) or more than (Q3 + 1.5*IQR)

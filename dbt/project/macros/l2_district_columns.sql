{% macro get_l2_district_types(scope="all") %}
    {#-
    L2 district column names, as a list.

    scope='all'       every district column in the L2 nationwide uniform file.
    scope='allocated' the curated list the district population allocation
                      unpivots (int__l2_block_district_map,
                      int__serve_block_coverage). Selection rule: every
    ...
    Returns:
        list: bare column names, no backticks.
    -#}
    {%- set all_types = [
    ...
    {%- else -%}
        {{
            exceptions.raise_compiler_error(
                "get_l2_district_types: unknown scope '"
                ~ scope
                ~ "'. Expected 'all' or 'allocated'."
            )
        }}
    {%- endif -%}
{% endmacro %}
...
{% macro get_l2_district_columns(
    ...
    Args:
        scope (str): passed through to get_l2_district_types.
        use_backticks (bool): true wraps names in backticks (SELECT); false
                              emits bare names (the UNPIVOT IN clause).

...
-- The in-context check is strictly tighter than plain membership: it requires
-- at least one matching candidacy_stage row whose election/position aligns
-- with the candidacy's (NULL-tolerant), preventing cross-race contamination
-- via upstream gp_candidacy_id collisions. Zero rows expected.
with

...
-- must be NULL. Archive-era candidacies cannot enter the population (archive
-- stage rows always carry 'hubspot'), and a NULL source_systems row counts as
-- evidence (fail open), matching the mart predicate. Zero rows expected.
with
    pledge_only_candidacies as (
        ...
        having
            count(*) = count_if(
                array_size(source_systems) = 1
                and array_contains(source_systems, 'gp_api')
            )
    )

select c.gp_candidacy_id, c.latest_stage_reached, c.source_systems
from {{ ref("candidacy") }} as c
inner join pledge_only_candidacies as p on c.gp_candidacy_id = p.gp_candidacy_id
where c.latest_stage_reached is not null

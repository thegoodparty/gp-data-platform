...
-- Matched on the shared normalized L2 key and scoped by state. L2 toggles a
-- " (EST.)" marker between snapshots, so raw-text matching re-raises an
-- already-classified value as drift, and an unscoped key would let one state's
-- parked value exempt another's.
left join
    {{ ref("proposed_district_ignored_values") }} as ignored
    on ignored.state = l2.state_postal_code
    and {{ normalize_l2_district_name("ignored.proposed_district") }}
    = {{ normalize_l2_district_name("l2.proposed_district") }}
where

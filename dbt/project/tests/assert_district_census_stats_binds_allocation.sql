...
-- (district_type='State') rows are census-derived, not from the allocation, so they
...
with
    allocation_rollup as (
        select
            ...
            district_type,
            district_name,
            sum(allocated_population) as allocation_pop,
            sum(voters_in_block_district) as allocation_voters,
            count(distinct block_geoid) as allocation_blocks
...
where
    ...
    or abs(m.district_population - s.allocation_pop) > 1e-6
    or m.registered_voters <> s.allocation_voters
    or m.n_census_blocks <> s.allocation_blocks

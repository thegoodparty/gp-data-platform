...
with
    stance_data as (
        select candidacy_id, stances from {{ ref("int__ballotready_stance") }}
    ),

    exploded_stances as (
        select candidacy_id, stance
        from stance_data
    ...
    invalid_stances as (
        ...
        where
            ...
            stance.issue.databaseid is null or stance.issue.id is null
    )

select *
from invalid_stances

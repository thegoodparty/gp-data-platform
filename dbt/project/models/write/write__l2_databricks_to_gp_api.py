...
PROTECTED_NONREMOVED_COLUMNS = [
    f'"{col}"' for col in VOTER_COLUMN_LIST if col not in REMOVED_COLUMNS
]


# build upsert query
voter_column_list_str = ",".join(PROTECTED_VOTER_COLUMN_LIST)
nonremoved_column_list_str = ",".join(PROTECTED_NONREMOVED_COLUMNS)
removed_column_null_list = "NULL AS " + ", NULL AS ".join(PROTECTED_REMOVED_COLUMNS)
...
# TODO: generate string of columns and concatenate
UPSERT_QUERY = (
    """
...
def _load_data_to_postgres(
    ...
    # chunk_size = 100_000  # 50,000 rows per chunk
    # total_rows = df.count()
    ...
    #     ).drop("row_id")
    #     chunk_df.write.format("jdbc") \
    #         .option("url", jdbc_url) \
    #         .option("dbtable", f'{staging_schema}."{table_name}"') \
    #         .option("user", db_user) \
...
def model(dbt, session: SparkSession) -> DataFrame:
    ...
    for state_id in state_list:
        ...
        # df = df.withColumn("Residence_Addresses_CheckDigit", col("Residence_Addresses_CheckDigit").try_cast("int"))
        # df = df.withColumn("Residence_Addresses_HouseNumber", col("Residence_Addresses_HouseNumber").try_cast("int"))
        # df = df.withColumn("Residence_Addresses_SuffixDirection", col("Residence_Addresses_SuffixDirection").try_cast("int"))
        # df = df.withColumn("Mailing_Addresses_CheckDigit", col("Mailing_Addresses_CheckDigit").try_cast("int"))
        # df = df.withColumn("Mailing_Addresses_HouseNumber", col("Mailing_Addresses_HouseNumber").try_cast("int"))
        # df = df.withColumn("Mailing_Addresses_PrefixDirection", col("Mailing_Addresses_PrefixDirection").try_cast("int"))
        # df = df.withColumn("Mailing_Addresses_SuffixDirection", col("Mailing_Addresses_SuffixDirection").try_cast("int"))

"""
Shared fuzzy matching utilities for use across dbt Python models.
This module provides reusable functions for fuzzy matching candidates
against HubSpot records.
"""
...
def get_fuzzy_match_schema(candidate_code_field_name: str = "candidate_code") -> StructType:
    """
...
def create_fuzzy_matching_udf(
    ...
    assert hubspot_code_field_name in hubspot_df.columns, f"Column {hubspot_code_field_name} not found in HubSpot DataFrame"
        ...
        for candidate_code in candidate_codes:
            ...
            for rank, (matched_code, score) in enumerate(matches, 1):
                if score >= threshold:
                    ...
                    fuzzy_results.append(_create_match_result(
                        candidate_code, matched_code, score, rank, hubspot_data, candidate_code_field_name
                    ))
                    match_found = True
                    break
...
def add_match_type_column(df, match_indicator_col: str = "fuzzy_matched_hubspot_candidate_code"):
    ...
    return df.withColumn(
        "match_type",
        when(col(match_indicator_col).isNotNull(), "fuzzy")
        .otherwise("none")
    )
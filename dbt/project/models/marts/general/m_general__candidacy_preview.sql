...
{% if election_match_count == 0 %}
    ...
    {% if is_incremental() %} select * from {{ this }} limit 0
    {% else %} select cast(null as string) as gp_candidacy_id
    {% endif %}
{% else %}
    ...
    with
        candidacies as (
            ...
            left join
                {{ ref("stg_model_predictions__candidacy_ddhq_matches_20251201") }}
                as tbl_ddhq_matches
            ...
            left join
                ...
                on tbl_ddhq_election_results_source.race_id
                = tbl_ddhq_matches.ddhq_race_id

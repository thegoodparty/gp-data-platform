...
--
-- Two cases:
-- - BR-anchored clusters: the canonical is BR's natural gp_election_stage_id,
-- so DDHQ/TS races collapse onto the BR race.
-- - Non-BR clusters (DDHQ <-> TS with no BR member): the canonical is one
-- member's own gp_election_stage_id (a deterministic representative), so the
-- matched DDHQ and TS races still collapse to a single stage. This is safe
-- re: the m_election_api br_position_database_id dup that
-- non_br_cluster_canonicals warns about, because these clusters have no BR
-- member (hence no br_position) and never reach that join. Single-record
-- clusters are omitted (nothing to collapse; they keep their natural id).
--

...
-- position via ballotready_position_id (= br_position_database_id), and
-- BR remains the authoritative source for descriptive election data.
-- This model emits one row per gp_election_id that gp_api participates
-- in. Descriptive columns are NULL by design; the mart's FOJ coalesce
-- picks BR's values.
--
-- Schema mirrors int__civics_election_techspeed for FOJ alignment in
-- the election mart.
with
    -- created_at / updated_at use min/max of contributing candidacy timestamps
    -- so gp_api-only rows have non-null audit timestamps at the mart layer

...
with
    ...
    -- BR API position table for br_position_level fallback.
    br_position as (
    ...
    joined as (
        ...
        select
            ...
            c.campaign_office as campaign_office_raw,  -- preserved for prematch
            c.normalized_position_name,  -- exposed for prematch
            c.election_level,
    ...
    -- compute candidate_office once in its own CTE so map_office_type can
    -- reference it as a real column, instead of nesting Jinja macros inside another
    -- macro's string argument.
    ...
    final as (
        select
            ...
            -- prefer BR's 7-bucket taxonomy when br_position_level is
            ...
            -- appended to preserve existing column order

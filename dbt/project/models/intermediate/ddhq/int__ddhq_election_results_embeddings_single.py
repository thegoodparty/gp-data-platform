...
class GeminiEmbeddingClient:
    ...
    def create_embeddings(
        self,
        texts: List[str],
        parallel: bool = False,  # Changed default to False for single process
        batch_size: int = 100,
        rate_limit_delay: float = 2.0,
        **kwargs,
    ) -> np.ndarray:
        ...
        Args:
            texts: List of texts to embed
            parallel: Whether to use parallel processing (ignored, always sequential)
...
def model(dbt, session: SparkSession) -> DataFrame:
    ...
    #     max_updated_at_row = existing_table.agg({"_airbyte_extracted_at": "max"}).collect()[0]
    ...
    # ddhq_name_race_texts = ddhq_name_race_texts[:10000]  # (100, 71 s), (1000, 410 s), (10k, 51m or 2.5k s)
    ...
    ddhq_name_race_embeddings_original: List[np.ndarray] = (
        embedding_client.create_embeddings(
            ...
            parallel=False,  # Use sequential processing
            batch_size=100,
        )
    )

...
def model(dbt, session: SparkSession) -> DataFrame:
    ...
    if dbt.is_incremental:
        ...
        existing_keys_df = (
            ...
            .select("gp_candidacy_id", "election_type", "election_date")
            .distinct()
        )
        candidacy_clean_for_ddhq = candidacy_clean_for_ddhq.join(
            existing_keys_df,
            on=[
                candidacy_clean_for_ddhq["gp_candidacy_id"]
                == existing_keys_df["gp_candidacy_id"],
                candidacy_clean_for_ddhq["election_type"]
                == existing_keys_df["election_type"],
                candidacy_clean_for_ddhq["election_date"]
                == existing_keys_df["election_date"],
            ],
            how="left_anti",
        )
    ...
    candidacy_name_race_embeddings: List[np.ndarray] = (
    ...
    if (
        isinstance(candidacy_name_race_embeddings, np.ndarray)
        and len(candidacy_name_race_embeddings.shape) == 2
    ):
        candidacy_name_race_embeddings = [
            candidacy_name_race_embeddings[i]
            for i in range(candidacy_name_race_embeddings.shape[0])
        ]

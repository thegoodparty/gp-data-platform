/* column list here must match what's in m_people_api__voter.sql */
{% set columns = [
...
with
    ...
    districts_from_voters as (
        {% for column in columns[:6] %}
            select
                id,
                "{{ column }}" as type,
                cast(`{{ column }}` as string) as name,
    ...
    add_district_id as (
        select
            tbl_district_voters.id as voter_id,
            tbl_district.id as district_id,
            tbl_district_voters.type,
            tbl_district_voters.name,
            tbl_district_voters.state
        from districts_from_voters as tbl_district_voters
        left join
            {{ ref("m_people_api__district") }} as tbl_district
            on tbl_district_voters.state = tbl_district.state
            and tbl_district_voters.type = tbl_district.type
            and tbl_district_voters.name = tbl_district.name
    )

select
    tbl_district_voters.voter_id,
    tbl_district_voters.district_id,
    tbl_district_voters.type,
    tbl_district_voters.name,
    tbl_district_voters.state,
    {% if is_incremental() %}
        -- For incremental runs, preserve existing created_at for existing
        -- records, set current timestamp for new records
...
from add_district_id as tbl_district_voters
{% if is_incremental() %}
    left join
        {{ this }} as tbl_existing
        on tbl_district_voters.voter_id = tbl_existing.voter_id
{% endif %}
where tbl_district_voters.district_id is not null

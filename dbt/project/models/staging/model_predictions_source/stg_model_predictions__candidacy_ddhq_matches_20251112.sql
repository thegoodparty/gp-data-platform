with
    source as (
        select *
        from {{ source("model_predictions", "candidacy_ddhq_matches_20251112") }}
    ),
    renamed as (
        select
            ...
            {{ adapter.quote("llm_prompt") }},
            {{ adapter.quote("llm_response") }},
            {{ adapter.quote("match_source") }},
            {{ adapter.quote("runoff_fallback_date") }},
            ...
            {{ adapter.quote("is_synthetic_runoff") }},
            {{ adapter.quote("discovered_from_ddhq_candidate_id") }}

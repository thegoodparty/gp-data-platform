...
- **Skills to load.** Read the `analytics-process` skill (how to work) and resolve every data concept through the product's knowledge skill (what is true; routed in step 1).

## Steps

1. **Route by product, and ask the profiling opt-in.** Resolve the product first — it selects the knowledge skill and working-set helper for every later step:

   | Product | Knowledge skill | Working-set helper |
   |---|---|---|
   | **Win** (candidates) | `win-analytics-knowledge` | `analytics/lib/win_analysis.py` |
   | **Serve** (elected officials) | `serve-analytics-knowledge` | `analytics/lib/serve_analysis.py` |

   If the question spans both products (nearly all Serve users are also Win candidates), frame it explicitly as cross-product and load both knowledge skills — do not silently pick one. Also ask the user once whether to **profile this run's pipeline cost** — default **no**; most runs skip it (see step 3). Record the answer.
...
- A concept resolves to several candidate metrics → resolve it through the product knowledge skill; check `dbt/project/models/**/sem_*.yml` `config.meta` first for the governed definition and owner, and `analytics/diagnostics/semantic_catalog/config/ratifications.yml` for the two sign-off halves, `business` (the rule) and `data` (the build, with `value_at_signing`); an absent half means that half is pending; `canonical_metrics.md` is a generated projection of both that can lag them.

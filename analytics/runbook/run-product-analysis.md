...
- A concept resolves to several candidate metrics → resolve it through the product knowledge skill; check `dbt/project/models/**/sem_*.yml` `config.meta` first for governed status (`canonical_metrics.md` is a generated projection of the semantic layer that can lag it).

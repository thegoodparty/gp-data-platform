...
| `has_completed_onboarding_flow` | bool | `onboarding_completed_at IS NOT NULL` (event `onboarding_complete`). **⚠ NEW-FLOW-BLIND** across the 2026-05-07 cutover (see §4). | Deprecated as a cohort filter; use **onboarding dashboard viewed** below. |
| `is_onboarded` | bool | US user, dashboard viewed within 14d of *Amplitude registration*. **⚠ NEW-FLOW-BLIND** across the 2026-05-07 cutover (anchor event died; see §4). | Do NOT use across the cutover; use **onboarding dashboard viewed** below. |
...
| **Onboarding milestone flags are new-flow-blind** | `onboarding_complete` / `has_completed_onboarding_flow` / `is_onboarded` / the lib `onboarded` flag read FALSE for post-cutover users, so a cross-cutover analysis shows a fake completion collapse. Mechanism, dates, and the verified 0.0% read are in §4. | Recompute **onboarding dashboard viewed** (broad) or `Onboarding - Candidate Pledge Completed` (strict) — both version-agnostic. See §4 + §3.5. |

...
# Create tmp_data directory if it doesn't exist, remove existing candidateposition.csv if it exists
mkdir -p ./tmp_data
rm -f ./tmp_data/candidateposition.csv
...
psql \
  ...
  -d "$db_name" \
  -c "$sql_command" \
  > ./tmp_data/candidateposition.csv

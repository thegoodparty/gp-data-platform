...
if [ "$is_incremental" = "true" ]; then
    sql_command="COPY (
        SELECT *
        FROM public.campaignplanversion
...
# Create tmp_data directory if it doesn't exist, remove existing campaignplanversion.csv if it exists
mkdir -p ./tmp_data
rm -f ./tmp_data/campaignplanversion.csv
...
psql \
  ...
  -d "$db_name" \
  -c "$sql_command" \
  > ./tmp_data/campaignplanversion.csv
...
# takes 13 minutes, 56 MB on disk

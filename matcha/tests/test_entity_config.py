...
def test_elected_official_config_comparisons():
    """Elected official config has 14 comparisons (adds the term_start_date
    election-cycle date comparison, the normalized-office comparison, and the
    office_category comparison for the ddhq source)."""
    from scripts.configs.elected_official import ELECTED_OFFICIAL_CONFIG

    assert len(ELECTED_OFFICIAL_CONFIG.comparisons) == 14

...
For the per-directory test hooks to pass on push, set up the environment of each directory you touch: `poetry install` in `dbt/`, `uv sync` in `people-api-loader/`, `airflow/`, `apps/genie-tools/`, and `apps/genie-slack-bot/`, and the root `requirements_test.txt` for `analytics/`. Each hook `cd`s into its directory and runs the suite via that env (`poetry run` / `uv run` / `pytest`), so you do not need to wrap `git` in any venv.

...
| `anthropic-analytics-loader/` | uv | 3.14 | Astral toolchain (ruff + ty), same as `people-api-loader/`. `uv sync --extra dev`, `uv run ...`. Console script `anthropic-analytics-loader`. Pulls the Claude Enterprise Analytics API into Delta tables for the Sigma "Claude spend" dashboard. |
...
For the per-directory test hooks to pass on push, set up the environment of each directory you touch: `uv sync` in `people-api-loader/`, `dbt/`, `airflow/`, `analytics/`, `apps/genie-tools/`, `reverse-etl/`, and `anthropic-analytics-loader/` (the last needs `--extra dev`). Each hook `cd`s into its directory and runs the suite via that env (`uv run`), so you do not need to wrap `git` in any venv.

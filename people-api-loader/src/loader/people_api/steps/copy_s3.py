...
def _truncate(conn: psycopg.Connection, table: str) -> None:
    with conn.cursor() as cur:
        cur.execute(f'TRUNCATE TABLE public."{table}"')  # ty:ignore[no-matching-overload]
...
def run(
    ...
    *,
    ...
    log.info(
        "copy.complete", uri=uri, all_loaded=all_loaded, covered=len(covered), expected=len(expected_tables)
    )
    return manifest

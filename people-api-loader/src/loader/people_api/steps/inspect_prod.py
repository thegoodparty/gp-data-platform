...
indexes diverge from that committed seed. Per-state counts are captured for the partitioned
tables, grouped by each table's real LIST-partition column (spec-driven, serving-column names:
both Voter and DistrictVoter use "State"); a flat table (District, DistrictStats) yields a total
count only.

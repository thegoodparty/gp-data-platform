...
Each step is one module under `src/loader/people_api/steps/` (`inspect_prod`, `unload`, `provision`, `create_schema`, `copy_s3`, `build_indexes`, `validate`, `resize`, `analyze`, `promote`, `teardown`), invoked as `loader <step> --date <ds_nodash>`. `promote` is the serving cutover: it writes the run's connection string to the single serving param `people-db-connection-string-{env}`, labels the new version `build-{date}`, and moves the `live` pointer people-api reads (`...:live`) onto it. State flows between steps through S3 manifests, not in-process handoff, so each step is independently runnable and re-entrant for a given run date (a step short-circuits if its manifest already exists). The DAG at `gp-data-platform/airflow/astro/dags/load_people_api.py` sequences them, one BashOperator per step.
...
- Don't omit the `Environment` tag (value = `ENVIRONMENT`, e.g. `prod` on the prod deployment) on any AWS resource the loader creates. The RDS-admin role's create policies condition on `RequestTag/Environment` matching the deployment env; missing or mismatched = denied API call.

...
**Databricks auth: one OAuth M2M credential, reused from a connection.** The DAG templates `DATABRICKS_HOST`, `DATABRICKS_CLIENT_ID`, `DATABRICKS_CLIENT_SECRET`, and `DATABRICKS_HTTP_PATH` from the shared Databricks Airflow connection selected by `LOADER_DATABRICKS_CONN_ID` (the same connection the L2 dbt jobs use: `databricks_dev` on dev, `databricks` on prod). Both `unload` (databricks-sdk) and the dbt gate read the same credential.

**Compute.** `unload` uses the Statement Execution API, which is warehouse-only (it cannot target an all-purpose cluster), so it runs on the SQL warehouse `LOADER_DATABRICKS_WAREHOUSE_ID`. The dbt-test gate instead reuses `DATABRICKS_HTTP_PATH` from the shared L2 connection, so it runs on whatever compute that connection points at (today the L2 all-purpose cluster). If the connection is ever pointed at a warehouse, the two align.

**Env vars go in Astro → Environment Variables (NOT Airflow Variables).** The loader reads `LOADER_*` / `DATABRICKS_*` from `os.environ`; the DAG's BashOperators forward them via `append_env=True`. Required: `LOADER_ENV`, `LOADER_S3_BUCKET`, `LOADER_S3_IMPORT_ROLE_ARN`, `LOADER_AWS_ACCOUNT_ID`, `LOADER_DATABRICKS_CONN_ID` (`databricks_dev` dev / `databricks` prod), `LOADER_DATABRICKS_WAREHOUSE_ID` (unload only; the gate uses the connection's http_path), `LOADER_VPC_ID`, `LOADER_DB_SUBNET_GROUP`, `LOADER_SECURITY_GROUP_ID`, `LOADER_KMS_KEY_ARN`, `LOADER_DBT_SCHEMA` (`dbt` prod / `dbt_staging` dev). The `DATABRICKS_*` values are templated from the connection by the DAG, not set directly. AWS creds (to assume the cross-account RDS-admin role) come from the worker's standard AWS credential chain.

...
1. **Prisma migration on `ZipToPosition`** — add three nullable columns: `voters_in_zip BigInt?`, `voters_in_zip_district BigInt?`, `pct_districtzip_to_zip Float?` (or match the repo's convention). Also add a composite index `@@index([zip_code, pct_districtzip_to_zip])` named `ZipToPosition_zip_code_pct_districtzip_to_zip_idx`. Both Prisma and the sync DAG (`_ztp_constraint_ddl`) declare this index — Prisma owns the prod-side definition; the DAG creates an identical index on its staging table during build so the atomic rename-swap (which expects every index in `ZTP.indexes` to exist on the live table) succeeds. This matches the existing pattern for the three other `ZipToPosition_*_idx` indexes.
...
4. **PR 2 (election-api repo)** — Prisma migration adds nullable columns AND the composite `(zip_code, pct_districtzip_to_zip)` index, API filter is **null-safe** — merges. Existing `ZipToPosition` rows in Postgres now have new columns set to null, but the null-safe filter (`pct_districtzip_to_zip IS NULL OR pct_districtzip_to_zip >= 0.005`) keeps the office picker working with the existing data. Confirm the Prisma migration created the index successfully before proceeding.
5. **Unpause the DAG.** Next run builds staging from `LIKE public.ZipToPosition` (which now has the new columns from step 4), bulk-inserts rows including the three new columns, applies all four indexes (the existing three plus the new composite) on the staging table via `_ztp_constraint_ddl`, then performs the atomic rename-swap. The swap renames every index in `ZTP.indexes` from canonical-name to `_old_`-prefixed on the live table — which works because Prisma created the new index in step 4. Postgres now has new columns populated; the API filter applies cleanly to non-null values.

The DAG's `create_staging_table` uses `CREATE TABLE staging.ZipToPosition_new (LIKE public.ZipToPosition INCLUDING DEFAULTS)`, so the staging table's column shape matches the live Postgres schema at build time. Step 4 ensures the live schema has the new columns before step 5 runs. The DAG's `_ztp_constraint_ddl` creates indexes on staging that mirror the live-side indexes Prisma owns; the swap renames them in lock-step.
